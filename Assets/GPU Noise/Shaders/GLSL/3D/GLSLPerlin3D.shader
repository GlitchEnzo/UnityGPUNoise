Shader "GPU Noise/GLSL/Perlin 3D" 
{
    Properties 
    {
        time ("time", float) = 0
    ...
    SubShader 
    {
        Pass 
        {
            ...
            void main()
            {				
                float result = perlin(vec3(TextureCoordinate * noiseScale, time)) * 0.5 + 0.5;
                //float result = perlin(TextureCoordinate * noiseScale) * 0.5 + 0.5;
