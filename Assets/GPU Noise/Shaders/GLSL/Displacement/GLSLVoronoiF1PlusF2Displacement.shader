Shader "GPU Noise/GLSL/Voronoi F1+F2 Displacement" 
{
    Properties 
    {
        time ("time", float) = 0
    ...
    SubShader 
    {
        Pass 
        {
            ...
            { 
				...
				float heightValue = voronoif1plusf2(vec3(texcoord * noiseScale)) * heightScale;
            ...
            void main()
            {                
                float result = voronoif1plusf2(vec3(TexCoord * noiseScale));
