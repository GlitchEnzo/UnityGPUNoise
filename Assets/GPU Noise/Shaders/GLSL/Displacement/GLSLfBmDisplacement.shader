Shader "GPU Noise/GLSL/fBm Displacement" 
{
    Properties 
    {
        time ("time", float) = 0
    ...
    SubShader 
    {
        Pass 
        {
            ...
            void main()
            { 
				float heightValue = fBm(vec4(gl_Normal * noiseScale, time)) * heightScale;
            ...
            void main()
            {                
                float result = fBm(vec4(Normal * noiseScale, time)) * 0.5 + 0.5;
