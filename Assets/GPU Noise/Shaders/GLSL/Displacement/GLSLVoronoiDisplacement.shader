Shader "GPU Noise/GLSL/Voronoi Displacement" 
{
    Properties 
    {
        time ("time", float) = 0
    ...
    SubShader 
    {
        Pass 
        {
            ...
            { 
				...
				float heightValue = voronoi(vec3(texcoord * noiseScale)).x * heightScale;
            ...
            void main()
            {                
                float result = voronoi(vec3(TexCoord * noiseScale)).x;
