Shader "GPU Noise/GLSL/Perlin Displacement" 
{
    Properties 
    {
        time ("time", float) = 0
    ...
    SubShader 
    {
        Pass 
        {
            ...
            { 
				Normal = gl_Normal;
				float heightValue = perlin(vec4(Normal * noiseScale, time)) * heightScale;
				//float heightValue = 0.25;
            ...
            void main()
            {                
                float result = perlin(vec4(Normal * noiseScale, time)) * 0.5 + 0.5;
