Shader "GPU Noise/GLSL/Ridged Displacement" 
{
    Properties 
    {
        time ("time", float) = 0
    ...
    SubShader 
    {
        Pass 
        {
            ...
            void main()
            { 
				float heightValue = ridged(vec4(gl_Normal * noiseScale, time)) * heightScale;
            ...
            void main()
            {                
                float result = ridged(vec4(Normal * noiseScale, time)) * 0.5 + 0.5;
