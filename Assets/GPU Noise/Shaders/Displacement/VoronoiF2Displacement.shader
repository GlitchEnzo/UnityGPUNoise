Shader "GPU Noise/Voronoi F2 Displacement" 
{
	Properties 
	{		
		time ("time", float) = 0
	...
	SubShader 
	{
		Pass 
		{
			...
			v2f vert (appdata_base v)
			{
				...
				float heightValue = voronoif2(float3(o.texcoord * noiseScale)) * heightScale; 
			...
			{
				float result = voronoif2(float3(i.texcoord * noiseScale));
