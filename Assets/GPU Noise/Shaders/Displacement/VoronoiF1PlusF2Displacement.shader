Shader "GPU Noise/Voronoi F1 + F2 Displacement" 
{
	Properties 
	{		
		time ("time", float) = 0
	...
	SubShader 
	{
		Pass 
		{
			...
			v2f vert (appdata_base v)
			{
				...
				float heightValue = voronoif1plusf2(float3(o.texcoord * noiseScale)) * heightScale;
			...
			{
				float result = voronoif1plusf2(float3(i.texcoord * noiseScale));
