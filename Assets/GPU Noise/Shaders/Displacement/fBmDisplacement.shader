Shader "GPU Noise/fBm Displacement" 
{
	Properties 
	{		
		time ("time", float) = 0
	...
	SubShader 
	{
		Pass 
		{
			...
			#pragma glsl
			//#pragma profileoption NumInstructionSlots=3000
			//#pragma profileoption NumMathInstructionSlots=3000
			#pragma vertex vert
			...
			{
				v2f o;
				float heightValue = fBm(float4(v.normal * noiseScale, time), 8.0) * heightScale;
			...
			{
				float result = fBm(float4(i.normal * noiseScale, time), 8.0) * 0.5 + 0.5;
