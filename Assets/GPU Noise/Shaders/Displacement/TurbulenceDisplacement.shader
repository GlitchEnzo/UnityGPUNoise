Shader "GPU Noise/Turbulence Displacement" 
{
	Properties 
	{		
		time ("time", float) = 0
	...
	SubShader 
	{
		Pass 
		{
			...
			{
				v2f o;
				float heightValue = turbulence(float4(v.normal * noiseScale, time), 8.0) * heightScale;
			...
			{
				float result = turbulence(float4(i.normal * noiseScale, time), 8.0) * 0.5 + 0.5;
